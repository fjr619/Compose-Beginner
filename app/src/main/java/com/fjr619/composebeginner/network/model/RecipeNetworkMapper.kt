...
import com.fjr619.composebeginner.domain.util.EntityMapper
...
class RecipeNetworkMapper: EntityMapper<RecipeNetworkEntity, Recipe> {
    ...
    fun fromEntityList(initial: List<RecipeNetworkEntity>): List<Recipe> = initial.map {
        mapFromEntity(it)
    }

    fun toEntityList(initial: List<Recipe>): List<RecipeNetworkEntity> = initial.map {
        mapToEntity(it)
    }
}